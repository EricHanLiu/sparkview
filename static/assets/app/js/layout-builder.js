  ...
  var e = {
      ...
      exportHtml: function() {
        $("#builder_export_html").click(function(t) {
          if (t.preventDefault(), envato.isVerified()) {
            var a = $(this);
            e.startLoad({
              ...
              var i = JSON.parse(t);
              if (void 0 !== i) var r = setInterval(function() {
                $.ajax("index.php", {
                  method: "POST",
                  data: {
                    builder_export: 1,
                  ...
                  var i = JSON.parse(t);
                  void 0 !== i && "1" === i.export_status && $("<iframe/>").attr({
                    src: "index.php?builder_export&builder_download&id=" + i.id,
  ...
  window.envato = {
    ...
    verifyEvent: function() {
      $("#builder_export").click(function(e) {
        e.preventDefault(), envato.isVerified() || (grecaptcha.reset(), $("#m-modal-purchase").modal("show"), $("#alert-message").addClass("m--hide"))
      }), $("#submit-verify").click(function(e) {
        e.preventDefault(), envato.reCaptchaVerified().done(function(e) {
          if (e.success) {
            var t = $("#purchase-code").val();
            envato.setItem("purchase_code", t), envato.verifyPurchase(t)
          } else grecaptcha.reset(), $("#alert-message").removeClass("alert-success m--hide").addClass("alert-danger").html("Invalid reCaptcha validation")
        })
      }), $("#purchase-code").keyup(function() {
        $("#alert-message").addClass("m--hide")
      })
    },
    verifyPurchase: function(e) {
      return $.ajax("https://marketplace.envato.com/api/edge/keenthemes/" + envato.apiKey + "/verify-purchase:" + e + ".json", {
        method: "GET"
      }).done(function(e) {
        $.isEmptyObject(e["verify-purchase"]) ? (grecaptcha.reset(), $("#alert-message").removeClass("alert-success m--hide").addClass("alert-danger").html("Invalid purchase code")) : (envato.setItems(e["verify-purchase"]), envato.setItem("verified", !0), $.ajax("http://keenthemes.com/metronic/preview/inc/api/envato.php", {
          method: "POST",
          data: envato.getItem()
        }), $("#alert-message").removeClass("alert-danger m--hide").addClass("alert-success").html("Purchase code is successfully verified"))
      }).fail(function(e) {
        grecaptcha.reset(), $("#alert-message").removeClass("alert-success m--hide").addClass("alert-danger").html("Invalid purchase code")
      })
    },
    setItems: function(e) {
